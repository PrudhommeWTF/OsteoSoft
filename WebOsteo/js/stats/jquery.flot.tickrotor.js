    ...
    function init(plot) {
        ...
        plot.hooks.draw.push(function (plot, ctx) {
            ...
            if (!secondPass) {
                ...
                /*
                var ticks2 = plot.getAxes().yaxis.ticks;
                for (var i = 0; i < ticks2.length; i++) {
                   elem = $('<span style="font:' + font + '">' + ticks2[i].label + '</span>');
                    plot.getPlaceholder().append(elem);
                    if (elem.outerWidth(true) > maxLabelWidth2) {
                        maxLabelWidth2 = elem.outerWidth(true);
                    }
                    elem.remove();
                }
                */
                ...
                for (var i = 0; i < ticks.length; i++) {
                    ...
                    if (rotateTicks <= 90) {
                        // Center such that the top of the label is at the center of the tick.
                        //xoffset = -Math.ceil(Math.cos(radsAboveHoriz) * tick.height) + maxLabelWidth2 - (tick.height/2) ; // décalage ici
                        //xoffset = -Math.ceil(Math.cos(radsAboveHoriz) * tick.height) + maxLabelWidth2; // décalage ici
                        xoffset = -Math.ceil(Math.cos(radsAboveHoriz) * tick.height) + 10; // décalage ici
