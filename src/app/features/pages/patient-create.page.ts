...
export class PatientCreatePage implements OnInit, OnDestroy {
  ...
  async generateConsultationSummaryPdf(): Promise<void> {
    ...
    try {
      ...
      let y = this.pdfBuilder.writeUnifiedHeader(pdf, {
        office,
        profile,
        officeFallbackName: this.consultationOfficeName()
      });
  ...
  async downloadConsentForm(): Promise<void> {
    ...
    try {
      ...
      const blob = this.pdfBuilder.buildConsentFormPdfBlob(office, profile, patientName);
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
